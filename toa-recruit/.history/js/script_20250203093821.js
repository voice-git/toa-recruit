  ...
  slides.forEach((slide) => {
    ...
    if (isInViewport) {
      ...
      gsap.to(slide, {
        ...
        scrollTrigger: {
          trigger: slide,
          start: "top 90%", // 要素の上端が画面の下から80%の位置に来たら開始
          end: "top 70%", // 要素の上端が画面の中央に来たら終了
          // toggleActions: "play none none none", // アニメーションを一度だけ再生
        },
      });
    }
  });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  interviewNotBars.forEach((interviewNotBar) => {
    const isInViewport = interviewNotBar.getBoundingClientRect().top < window.innerHeight;
...
(() => {
  ...
  const triggerOffset = .7; // 0.9や1に設定可能
