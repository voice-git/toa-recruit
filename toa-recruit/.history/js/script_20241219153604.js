document.addEventListener("DOMContentLoaded", function () {
  ...
  //   // モーダルを閉じる際の処理
  // MicroModal.close = function (modalId) {
  //   const modal = document.getElementById(modalId);
  //   if (modal) {
  //       // modal.classList.add('no-animation'); // no-animation クラスを追加
  //       // modal.setAttribute('aria-hidden', 'true');
  //       modal.querySelector('.p-modal__overlay').style.animation = 'none'; // アニメーションを無効化
  //   }
  // };

  // // モーダルを開く際の処理
  // MicroModal.show = function (modalId) {
  //   const modal = document.getElementById(modalId);
  //   if (modal) {
  //       // modal.classList.add('no-animation'); // no-animation クラスを追加
  ...
  function setupAnimationForContent01Pc() {
    ...
    gsap.utils.toArray(".js-not").forEach((notEffect) => {
      ...
      // gsap.set([".o-part-content-01"], {
      //   scaleX: 1 / 7.83,
      //   transformOrigin: "center center",
      // });
      // gsap.set([".n-part-content-01"], {
      //   x: 227.71,
      // });
      // gsap.set([".t-part-content-01"], {
      //   x: -229.3,
      // });
  ...
  function setupAnimationForContent01Sp() {
    gsap.utils.toArray(".js-not-sp").forEach((notEffect) => {
      const oPart = notEffect.querySelector(".o-part-content-01-sp");
      const nPart = notEffect.querySelector(".n-part-content-01-sp");
      const tPart = notEffect.querySelector(".t-part-content-01-sp");
      const svg = notEffect.querySelector(".not-svg-content-01-sp");
  ...
  function setupAnimationForContent02Pc() {
    ...
    gsap.utils.toArray(".js-not-content-02-normal").forEach((notEffect) => {
      // アニメーションのタイムラインを作成
      const tl = gsap.timeline({
        scrollTrigger: {
          trigger: notEffect, // 個別のトリガー要素を指定
      ...
      gsap.set([".o-part.is-content-02"], {
        scaleX: 1 / 7.83,
        transformOrigin: "center center",
      });
      gsap.set([".n-part.is-content-02"], {
        x: 215.71,
      });
      gsap.set([".t-part.is-content-02"], {
        x: -217.3,
      });
      ...
      tl.to(
        notEffect.querySelector(".o-part.is-content-02"),
        {
          duration: 1, // 横幅の拡大（scaleX）は3秒で行う
          scaleX: 1, // 横幅を拡大（611.34 / 107.87 ≈ 5.66）
          transformOrigin: "center center", // 中心を基準に拡大
      ...
      tl.to(
        notEffect.querySelector(".n-part.is-content-02"),
        {
          duration: 1, // 横幅の拡大（scaleX）は3秒で行う
          x: 0, // 左へ移動
          ease: "power1.inOut",
      ...
      tl.to(
        notEffect.querySelector(".t-part.is-content-02"),
        {
          duration: 1, // 横幅の拡大（scaleX）は3秒で行う
          x: 0, // 右へ移動
          ease: "power1.inOut",
  ...
  function setupAnimationForContent02Sp() {
    gsap.utils.toArray(".js-not-content-02-normal").forEach((notEffect) => {
      ...
      gsap.set([".o-part.is-content-02-sp"], {
        scaleX: 1 / 7.83,
        transformOrigin: "center center",
      });
      gsap.set([".n-part.is-content-02-sp"], {
        x: 125.71,
      });
      gsap.set([".t-part.is-content-02-sp"], {
        x: -127.3,
      });

      // SVG全体にdrop-shadowを適用して影を徐々に強くする
      tl.to(
        notEffect.querySelector(".js-not-svg.is-content-02-sp"),
        {
          duration: 1,
          filter: "drop-shadow(0px 0px 22.68px rgba(255, 255, 255, 1))",
          ease: "power1.inOut",
      ...
      tl.to(
        notEffect.querySelector(".o-part.is-content-02-sp"),
        {
          duration: 1, // 横幅の拡大（scaleX）は3秒で行う
          scaleX: 1, // 横幅を拡大（611.34 / 107.87 ≈ 5.66）
          transformOrigin: "center center", // 中心を基準に拡大
      ...
      tl.to(
        notEffect.querySelector(".n-part.is-content-02-sp"),
        {
          duration: 1, // 横幅の拡大（scaleX）は3秒で行う
          x: 0, // 左へ移動
          ease: "power1.inOut",
      ...
      tl.to(
        notEffect.querySelector(".t-part.is-content-02-sp"),
        {
          duration: 1, // 横幅の拡大（scaleX）は3秒で行う
          x: 0, // 右へ移動
          ease: "power1.inOut",
  ...
  function setupAnimationForContent02LargePc() {
    // スクロールすると発火するNOTアニメーション(content 02)
    gsap.utils.toArray(".js-not-content-02-large").forEach((notEffect) => {
      // アニメーションのタイムラインを作成
      const tl = gsap.timeline({
        scrollTrigger: {
          trigger: notEffect, // 個別のトリガー要素を指定
  ...
  function setupAnimationForContent02SmallPc() {
    // スクロールすると発火するNOTアニメーション(content 02)
    gsap.utils.toArray(".js-not-content-02-small").forEach((notEffect) => {
      // アニメーションのタイムラインを作成
      const tl = gsap.timeline({
        scrollTrigger: {
          trigger: notEffect, // 個別のトリガー要素を指定
  ...
  gsap.utils.toArray(".js-interview").forEach((notEffect) => {
    ...
    ScrollTrigger.create({
      ...
      onLeaveBack: () => createAnimation(notEffect), // 戻るときもアニメーションを再実行
    });
  });
});
