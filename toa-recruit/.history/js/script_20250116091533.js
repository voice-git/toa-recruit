document.addEventListener("DOMContentLoaded", function () {
  ...
  const closeMenu = function () {
    const isHovering =
      hamburger.matches(":hover") ||
      mobileMenu.matches(":hover") ||
      mobileheaderheader.matches(":hover");
    if (!isHovering) {
      hamburger.classList.remove("is-open");
      mobileMenu.classList.remove("is-show");
    }
  ...
  // ハンバーガーメニューにホバーしたときメニューを開く
  hamburger.addEventListener("mouseenter", openMenu);

  // ウィンドウ全体でマウスが移動したとき、メニュー外をチェック
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            // .is-unfixedClass を通過して .p-message に戻ったらクラスを削除
