...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const interviewNot = document.querySelectorAll(".js-interviewNot");
  ...
  slides.forEach((slide) => {
    ...
    if (isInViewport) {
      ...
      if (interviewNot.lenght > 0) {
        tl.to({}, { duration: 1}); // 1秒待機

        interviewNot.forEach((interviewNot))
        tl.fromTo (
          interviewNot,
          {
            clipPath: "cubic-bezier(0.86, 0, 0.07, 1);",
          },
          {
            clipPath: "inset(0 0 0 0)", // アニメーション終了状態
            ease: "power2.out",
