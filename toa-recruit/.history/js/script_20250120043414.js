document.addEventListener("DOMContentLoaded", function () {
  ...
  // const main = document.querySelector(".l-footer");
  let state = false;
  let pos;
        ...
        document
          ...
          .forEach((button) => {
            button.addEventListener("click", function () {
              ...
              // モーダルが閉じられ、次のモーダルが開くタイミングでスクロール位置をリセット
...
// document.querySelectorAll(".p-features__itemTrigger").forEach((button) => {
//   button.addEventListener("click", function (e) {
//     e.preventDefault(); // クリック時のデフォルト動作（スクロール）を防ぐ

//     // 現在のスクロール位置をログ出力
//     console.log('Current scrollY:', window.scrollY);

//     const modalId = this.dataset.micromodalTrigger;
...
//       // モーダルが表示された後にスクロール位置を元に戻す
//       setTimeout(() => {
//         window.scrollTo(0, scrollPosition); // 保存したスクロール位置に戻す
//       }, 100); // 少し遅延を入れて、モーダルが表示されてから戻す
//     }
//   });
// });
...
// $(function () {
//   $.scrollify({
//     section: ".js-scrollify", //対象要素を指定
//     easing: "swing", // イージングを指定
//     scrollSpeed: 600, // スクロール時の速度
//     setHeights: false,
//     // updateHash: false, // スクロール時アドレスバーのURLを更新

//     interstitialSection: ".l-entry, .l-footer", // 通常スクロールセクション
//     standardScrollElements: '.p-interview__area--item, .p-interview__content',
//     // standardScrollElements: '.p-interview__area--item, .p-interview__content', //通常スクロール要素
//     // standardScrollElements: '.p-interview__itemOuter-1, .p-interview__content--1, .p-interview__itemOuter-2, .p-interview__content--2, .p-interview__itemOuter-3, .p-interview__content--3, .p-interview__itemOuter-4, .p-interview__content--4, .p-interview__itemOuter-5, .p-interview__content--5, .p-interview__itemOuter-6, .p-interview__content--6, .p-interview__itemOuter-7, .p-interview__content--7, .p-interview__itemOuter-8, .p-interview__content--8, .p-interview__itemOuter-9, .p-interview__content--9', //通常スクロール要素
...
(() => {
	...
	const hasInertialScroll = "ontouchstart" in window || navigator.maxTouchPoints > 0 || navigator.msMaxTouchPoints > 0;
	...
	function checkSections() {
		...
		sections.forEach((section) => {
			...
			if (sectionTop < windowBottom && sectionBottom > windowTop) {
				...
				const visibleHeight = Math.min(windowBottom, sectionBottom) - Math.max(windowTop, sectionTop);
		...
		if (targetSection && !isSnapping) {
			...
			const targetPosition = isScrollingDown ? sectionTop : sectionBottom - windowHeight;
...
// $.scrollify({
//   section: ".js-scrollify", // スナップ対象のセクション
//   easing: "swing", 
//   scrollSpeed: 600, 
//   updateHash: false, 
//   standardScrollElements: '.p-interview__content', // 標準スクロール対象
//   setHeights: false, // 各セクションの高さを Scrollify によって固定させない
//   overflowScroll: true, // 標準スクロール要素内でのオーバーフロースクロールを許可
// });
...
// $(function () {
//   $.scrollify({
//     section: ".js-scrollify", // スクロールスナップを適用する要素
//     easing: "swing", // スクロールの動き
//     scrollSpeed: 600, // スクロール速度
//     updateHash: false, // URLの更新を無効化
//     standardScrollElements: '.p-interview__content', // 通常スクロールを適用する要素
//     before: function (index, sections) {
//       const currentSection = sections[index];

//       // スクロールスナップを切り替え
//       if (
//         $(currentSection).hasClass('p-interview__content') ||
//         $(currentSection).prev().hasClass('p-interview__content')
//       ) {
//         $.scrollify.disable(); // 通常スクロールに切り替え
//       } else {
//         $.scrollify.enable(); // スクロールスナップを再有効化
//       }
//     },
//   });

//   // スクロールスナップを有効化するためのリセット機能
//   $('.p-interview__content').on('mouseleave', function () {
//     $.scrollify.update(); // Scrollify の状態を再計算
//   });
// });
...
//       // // 親要素のoverflowを変更して通常スクロールを許可
//       // const swiperContainer = document.querySelector('.swiper-interview');
//       // swiperContainer.style.overflow = 'visible';
//       // swiperContainer.style.height = 'auto'; // Swiper全体を通常スクロール可能に

//   //     // Swiperから通常のスクロールに移行
//   // const nextContent = document.querySelector('.p-interview__content');
...
//   // Swiperのスクロール操作を無効化
//   this.disable(); // Swiperの操作を完全無効化
//     },
//     reachBeginning: function () {
//       // 必要に応じて、逆方向でも制御を解除できます
//     },
//   },
// });

// // 初期化時にSwiper領域のoverflowをhiddenに
// document.querySelector('.swiper-interview').style.overflow = 'hidden';

// // // マウスホイールのイベントをカスタマイズして、最後のスライド後に通常スクロールを有効化
// // document.querySelector('.swiper-interview').addEventListener('wheel', (event) => {
// //   if (interviewSwiper.isEnd) {
// //     document.querySelector('.swiper-interview').style.overflow = 'visible';
// //   }
// // });

// document.addEventListener("DOMContentLoaded", () => {
//   const snapWrapper = document.querySelector('.p-interview__scrollWrapper');
//   const snapAreas = document.querySelectorAll('.p-interview__area');

//   if (snapWrapper && snapAreas.length > 0) {
//     // IntersectionObserverの設定
//     const observerOptions = {
//       root: null, // ビューポートを基準
//       // threshold: 0.9, // 90%以上が画面に入ったら発火
//       threshold: 1,
//     };
...
//         if (entry.isIntersecting) {
//           // スクロールスナップを有効化
//           snapWrapper.style.scrollSnapType = 'y mandatory';
//         } else {
//           // スクロールスナップを無効化
...
//     // 各スクロールスナップ対象にObserverを設定
//     snapAreas.forEach(area => observer.observe(area));
//   }
// });
