        ...
        <div class="p-job-detail__inner c-inner">
            ...
            <div class="p-job-detail__content">
                <ul>
                    ...
                    <?php
                    $job_image = get_field('job_detail_image');
                    $job_headline = get_field('job_detail_headline');
                    $job_text = get_field('job_detail_text');

                    // 条件分岐: いずれかの値が存在する場合のみ出力
                    if ($job_image || $job_headline || $job_text): ?>
                    <li>
                        <div>
                            <?php if ($job_headline): // 見出しがある場合 ?>
                                <h3>
                                    <?php echo nl2br($job_headline); ?>
                                </h3>
                            <?php endif; ?>
                            <?php if ($job_text): // テキストがある場合 ?>
                                <p>
                                    <?php echo nl2br($job_text); ?>
                                </p>
                            <?php endif; ?>        
                        </div>
                        <div>
                            <?php if ($job_image && is_array($job_image)): // 画像がある場合 ?>
                                <img src="<?php echo esc_url($job_image['url']); ?>" alt="<?php echo esc_attr($job_image01['alt']); ?>">
                            <?php endif; ?>
                        </div>
                    </li>
                    <?php endif; ?>
